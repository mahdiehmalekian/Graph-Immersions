"""
Contains functions which check whether a graph has types A_k, B_4.
...
def is_type_A_k(G, u, v, k, rest=None, checked_deg_uv=None, checked_deg_rest=None):
    ...
    for every 1 <= i <= |V|-2"""
    if checked_deg_uv is None:
        if G.deg(u) != k or G.deg(v) != k:
            return [False]
    if checked_deg_rest is None:
        for x in G.vertices():
...
def is_type_B_4(H, root, root_p, connected=None, two_econ=None):
